...
// @version      5.0
// @description  点击图片勾选，单图复制/下载，轻提示反馈
// @author       YourName
...
(function() {
    ...
    function extractImageLinks() {
        ...
        imgs.forEach(img => {
            ...
            if (src && !src.includes('s.gif') && !/icon/i.test(src)) links.push(src);
        });
        if (links.length === 0) {
    ...
    function createPanel() {
        ...
        const createToolBtn = (text, onClick, color = '#ff8a5c') => {
            ...
            `;
            btn.onmouseenter = () => {
                btn.style.backgroundColor = darken(color, 15);
            };
            btn.onmouseleave = () => {
        ...
        const baseColor = '#ff8a5c'; // 柔和橙

        const selectAllBtn = createToolBtn('全选', () => {
            document.querySelectorAll('.image-card').forEach(card => card.dataset.selected = 'true');
            updateSelectionUI();
        }, baseColor);
        const deselectAllBtn = createToolBtn('取消全选', () => {
            document.querySelectorAll('.image-card').forEach(card => card.dataset.selected = 'false');
            updateSelectionUI();
        }, baseColor);
        const copyLinksBtn = createToolBtn('复制链接', () => {
        ...
        const downloadSelectedBtn = createToolBtn('下载选中', () => {
            const selected = getSelectedUrls();
            if (selected.length === 0) {
        ...
        const downloadAllBtn = createToolBtn('全部下载', () => {
            const all = document.querySelectorAll('.image-card');
            const urls = Array.from(all).map(card => card.dataset.url);
            if (!urls.length) return;
            showToast(`⏳ 正在下载 ${urls.length} 张...`, 1200);
            downloadImages(urls);
        }, baseColor);
        const refreshBtn = createToolBtn('🔄 刷新', () => {
            loadImages();
        }, '#7f8c8d'); // 灰色
        ...
        function updateSelectionUI() {
            document.querySelectorAll('.image-card').forEach(card => {
                ...
                imgWrapper.style.outline = isSelected ? '4px solid #2196F3' : 'none';
                imgWrapper.style.outlineOffset = '-2px';
                imgWrapper.style.boxShadow = isSelected ? '0 0 0 4px rgba(33,150,243,0.3)' : 'none';
            });
        }
